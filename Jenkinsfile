pipeline {
  ...
  tools { 
   ...
   stages{
	...
	stage('Build') { 
            steps { 
               withDockerRegistry([credentialsId: "dockerloginn", url: ""]) {
                 script{
                 app =  docker.build("chethan")
                 }
               }
            }
    }
	...
	stage('Push') {
            steps {
                script{
                    docker.withRegistry('https://270403983022.dkr.ecr.us-west-2.amazonaws.com', 'ecr:us-west-2:aws-credentials') {
                    app.push("latest")
                    }
                }
            }
